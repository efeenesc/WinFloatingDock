...
namespace WinFloatingDock {
	namespace manager {
		 ...
		 *
		 * This has been made a bit redundant now that Program Manager and Windows Input Experience are correctly filtered out
		 * by the IsAltTabWindow function. This may serve some purpose in the future
		 */
		...
		/**
		 * @brief Attempts to load icon of the given HWND by trying multiple methods.
		 *
		 * Attempts to get large icon from window; then attempts to get small icon;
		 * then large class icon, and then small class icon. If getting icon from window fails,
		 * attempts to get icon from the executable by finding the executable first. If that too
		 * fails, returns the default program HICON.
		 *
		 * @param HWND of the window to extract icon from
		 * @return HICON of the window or the default Windows program HICON (IDI_APPLICATION)
		 */
