...
namespace WinFloatingDock {
    namespace window {
        ...
        const COLORREF HOVERCOLOR = RGB(58, 58, 58);
        const COLORREF HANDLECOLOR = RGB(154, 154, 154);
        const COLORREF CLOCKBGCOLOR = RGB(65, 65, 65);
        const COLORREF CLOCKTEXTCOLOR = RGB(220, 220, 220);
        ...
        void Draw(HDC hdc, const std::vector<manager::WindowInfo>& windows, const RECT& updateRect) {
            ...
            if (updateRect.left < clockRect.right && updateRect.right > clockRect.left) {
                DrawClock(hdc, clientRect, currentXPos, nullptr);
            }
        }
        ...
        void DrawIcons(HDC hdc, const std::vector<manager::WindowInfo>& windows, RECT& clientRect, long startingX, long* endingX) {
            ...
            int iconSize = 32;
            int iconPadding = 14;
            HPEN handlePen = CreatePen(PS_SOLID, 0, HOVERCOLOR);
            SelectObject(hdc, handlePen);
        ...
        void DrawIcon(HDC hdc, const manager::WindowInfo& window, const RECT& iconRect, bool isHovered) {
            ...
            if (isHovered) {
                HBRUSH hoverBrush = CreateSolidBrush(HOVERCOLOR);
                HPEN hoverPen = CreatePen(PS_SOLID, 0, HOVERCOLOR);
                SelectObject(hdc, hoverBrush);
        ...
        void DrawClock(HDC hdc, RECT& clientRect, long startingX, long* endingX) {
            ...
            int clockWidth = 50;
            int clockHeight = clientRect.bottom;
            int clockX = clientRect.right - clockWidth; // 10px padding from right edge
            ...
            HBRUSH clockBrush = CreateSolidBrush(CLOCKBGCOLOR);
            HPEN clockPen = CreatePen(PS_SOLID, 0, CLOCKBGCOLOR);
            SelectObject(hdc, clockBrush);
            SelectObject(hdc, clockPen);
            Rectangle(hdc, clockRect.left, clockRect.top, clockRect.right, clockRect.bottom);
            ...
            SetTextColor(hdc, CLOCKTEXTCOLOR);
